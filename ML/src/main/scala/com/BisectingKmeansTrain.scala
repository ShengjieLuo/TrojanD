...
object BisectingKmeansTrain{

  def main(args : Array[String]):Unit = {
    val sparkconf = new SparkConf().setAppName("BisectingKMeansTrain")
    val sc = new SparkContext(sparkconf)

    //Step1 : Load the data & Save the model
    val data = sc.textFile("file:///usr/local/TrojanD/sample/MLlib.txt")
    val parsedData = data.map(s => Vectors.dense(s.split(' ').map(_.toDouble))).cache()
    val model = new BisectingKMeans().setK(15).run(parsedData)
    model.save(sc, "file:///usr/local/TrojanD/BisectingKMeansModel")
    ...
    val problemWriter = new PrintWriter(new File("../MetaData/BisectingKMeansModel.txt"))
